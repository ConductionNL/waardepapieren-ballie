# waardepapieren-balie chart

The Helm chart installs waardepapieren-balie and by default the following dependencies using subcharts:
...
```bash
helm repo add waardepapieren-balie https://raw.githubusercontent.com/ConductionNL/waardepapieren-ballie/master/api/helm/
helm repo update
```
...
```bash
helm install my-waardepapieren-balie ConductionNL/waardepapieren-balie --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `waardepapieren-balie` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `waardepapieren-balie` |
